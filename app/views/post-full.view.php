   ...
   <div class="row post p-1">
      <div class="col-3 bg-light text-center">
         <a href="<?= ROOT ?>/profile/<?= $post->user->id ?>">
            ...
            <img class="profile-image rounded-circle m-1" src="<?= get_image($post->user->image ?? '') ?>" alt="profile" style="width: 80px;height:80px;object-fit:cover;">
      ...
      <div class="col-9 text-start">
         ...
         <?php if (!empty($post->image)): ?>
            <a href="<?= ROOT ?>/post/<?= $post->id ?>">
               <img class="my-1" src="<?= get_image($post->image) ?>" alt="profile" style="width: 100%;">
            </a>
         <?php endif; ?>
      ...
      <!--comment area-->
      <div>
         <form action="" method="post" onsubmit="submit_comment(event)">
            <div class="bg-secondary p-2">
               <textarea rows="4" id="comment-input" class="form-control" placeholder="Type a comment here?" name="post_content"></textarea>
               <input type="hidden" id="post_id" value="<?=$post->id?>">
               <label>
                  <i style="cursor:pointer;" class="h1 text-white bi bi-image"></i>
                  <input id="comment-image-input" onchange="display_comment_image(this.files[0]);" type="file" class="d-none" name="profile_image">
               </label>
               <button class="mt-1 btn btn-warning float-end">comment</button>
               <div class="text-center d-none">
                  <img class="comment-image m-1" alt="profile" style="width: 100px; height:100px;object-fit:cover;">
               </div>
