...
<div class="p-2 col-md-6 shadow mx-auto border rounded">
   <div class="text-center">
      ...
      <span>
         <img class="profile-image rounded-circle m-4" src="<?= get_image($row->image) ?>" alt="profile" style="width: 200px;height:200px;object-fit:cover;">
         <?php if (user('id') == $row->id): ?>
            <label>
               <i style="position: absolute; cursor:pointer;" class="h1 text-primary bi bi-image"></i>
               <input onchange="display_image(this.files[0]);" type="file" class="d-none" name="profile_image">
            </label>
         <?php endif; ?>
      </span>
      ...
      <form action="" method="post" onsubmit="submit_post(event)">
         <input value="<?= old_value('username', $row->username) ?>" type="text" class="form-control" placeholder="Username" name="username" id="username">
         <input value="<?= old_value('email', $row->email) ?>" type="email" class="form-control" placeholder="Email" name="email" id="email">
         <input value="<?= old_value('password', '') ?>" type="password" class="form-control" placeholder="Password (leave empty to keep old one)" name="password" id="password">
